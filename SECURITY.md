...
- No `Invoke-Expression`, downloaded script execution, signature bypass, downgrade flag, app-data reset, registry repair or blanket AppX re-registration.
- Windows may close target-package processes only after the user chooses 1 or supplies `-CloseRunningApp`. This uses `ForceTargetApplicationShutdown`, never the broader `ForceApplicationShutdown` or name-based process killing. It can interrupt ChatGPT / Codex tasks and unsaved work. The default choice waits for manual exit; `-NoPause` alone never grants shutdown permission. Diagnose and Download modes never close apps.
- Process discovery queries the exact package family with Windows limited-query access, with a publisher-specific package-path fallback for helpers. It does not read process memory or rely on a process name. Paths are not added to reports.
